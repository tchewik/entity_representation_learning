...
Finds 'nan' token instead of @@UNKNOWN@@; it is a workaround for allennlp predict that for some reason wants
to read labels in the labeled test file. Those labels are none of its business.
If you are bothered by
    raise KeyError(
    KeyError: "'blah-blah' not found in vocab namespace 'labels5', and namespace does not contain the default OOV token
during allennlp predict ...,
just copy it into the $pythonpath/site-packages/allennlp/data/vocabulary.py
(the file that bothers you with an exception)
"""
    ...
    def get_token_index(self, token: str, namespace: str = "tokens") -> int:
        try:
            ...
            try:
                ...
                try:
                    return self._token_to_index[namespace]['nan']
                except KeyError:
