...
FT_TRAIN = 'train_ft_flat.txt'
FT_VAL = 'validation_ft_flat.txt'
DATA_DIR = 'data'


class FasttextFlatClassifier(HierarchicalClassifier):
    ...
    def evaluate(self):
        ...
        for fold in tqdm(folds, desc=Fore.BLUE + f'Processing {self.archive}' + Style.RESET_ALL):
            ...
            model_path = os.path.join(subdir, f'model_q_{self.archive}_flat.fasttext')
            ...
            predictions = [model.predict(text)[0][0].split('--')[1:] for text in test.title.map(
                lambda row: ' '.join(wordpunct_tokenize(row.lower())))]

            pred_branches = [' | '.join(pred) for pred in predictions]
            pred_lvls = [[pred[i] for pred in predictions] for i in range(self._max_lvl)]
    ...
    def fit(self, train, val, mappers, max_iter=3):
        ...
        val_tokens_ft = val.title.map(lambda row: ' '.join(wordpunct_tokenize(row.lower())))
        ...
        for i in range(max_iter):
            ...
            try:
                ...
                model_predictions = [_model.predict(text)[0][0] for idx, text in val_tokens_ft.items()]
                pred_lvl1 = [pred.replace('__label__--', '').split('--')[0] for pred in model_predictions]
    ...
    def _make_tmp_file(self, data, mappers):
        ...
        # label = ft_labels.map(lambda row: '__label__' + '/'.join(row.split()))
        _data = pd.DataFrame({
    ...
    def load_the_save(self, fold):
        return fasttext.load_model(
            os.path.join(self._model_dir, f'fold_{fold}', f'model_q_{self.archive}_flat.fasttext'))
...
def evaluate(archive, n_folds=5, folds=None, retrain=False, n_top_nodes=7, max_lvl=5, lr=.01):
    """ This will perform the following steps:
        1) Construct the classifier
        2) Train if:
            a) there are no models under models/c_fasttext_flat/fold_*/model_q_{archive}_flat.fasttext
            b) flag --retrain is present
        3) Take the trained models, predict branches on test, and evaluate properly.
    ...
    """
    clf = FasttextFlatClassifier(archive, n_top_nodes=n_top_nodes, retrain=retrain, n_folds=n_folds, folds=folds,
                                 max_lvl=max_lvl, lr=lr)
    clf.evaluate()


if __name__ == '__main__':
    # python src/classifiers/c_fasttext_flat.py Industrial_and_Scientific --retrain
    fire.Fire(evaluate)
